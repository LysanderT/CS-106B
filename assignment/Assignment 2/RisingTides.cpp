...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> if_flood(terrain.numRows(),terrain.numCols(),false);
    Queue<GridLocation> to_access;
    for (GridLocation source: sources){
        if (terrain[source.row][source.col] <= height){
            if_flood[source.row][source.col] = true;
            to_access.enqueue({source.row,source.col});
        }
    }
    while (!to_access.isEmpty()){
        GridLocation accessing = to_access.dequeue();
        if (accessing.row-1 >= 0){
            if (if_flood[accessing.row-1][accessing.col] == false){
                if (terrain[accessing.row-1][accessing.col] <= height){
                    if_flood[accessing.row-1][accessing.col] = true;
                    to_access.enqueue({accessing.row-1,accessing.col});
                }
            }
        }
        if (accessing.col-1 >= 0){
            if (if_flood[accessing.row][accessing.col-1] == false){
                if (terrain[accessing.row][accessing.col-1] <= height){
                    if_flood[accessing.row][accessing.col-1] = true;
                    to_access.enqueue({accessing.row,accessing.col-1});
                }
            }
        }
        if (accessing.row + 1 < terrain.numRows()){
            if (if_flood[accessing.row+1][accessing.col] == false){
                if (terrain[accessing.row+1][accessing.col] <= height){
                    if_flood[accessing.row+1][accessing.col] = true;
                    to_access.enqueue({accessing.row+1,accessing.col});
        ...
        if (accessing.col + 1 < terrain.numCols()){
            if (if_flood[accessing.row][accessing.col+1] == false){
                if (terrain[accessing.row][accessing.col+1] <= height){
                    if_flood[accessing.row][accessing.col+1] = true;
                    to_access.enqueue({accessing.row,accessing.col+1});
