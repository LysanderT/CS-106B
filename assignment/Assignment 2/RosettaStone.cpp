...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if (kGramLength <= 0){
        error("the given kGramLength is negative or equals to zero.");
    }
    if (str.length() < kGramLength){
        return dict;
    }
    for (int i = 0; i + kGramLength <= str.length(); i++){
        string item = str.substr(i,kGramLength);
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key:input){
        denm += pow(input[key],2);
    }
    denm = sqrt(denm);
    if (denm == 0){
        error("The inputs are all zeroes.");
    }
    for (string key:input){
        double value = input[key]/denm;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if (numToKeep >= source.size()){
        ...
        error("negative numToKeep is not feasible");
    }
    PriorityQueue<string> helper;
    for (string key: source){
        helper.enqueue(key, source[key]);
    }
    for (int i = 0; i + numToKeep < source.size(); i++){
        helper.dequeue();
    }
    Map<string,double> tops;
    while(!helper.isEmpty()){
        string key = helper.dequeue();
        tops.put(key, source[key]);
    }
    return tops;
}
